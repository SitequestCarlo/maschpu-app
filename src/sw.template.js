...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    (async () => {
      if (isDevHost) {
        console.log(
          "[SW] Dev host detected, skipping precache to keep assets live",
        );
        self.skipWaiting();
        return;
      }
      try {
        ...
        console.log(
          `[SW] Installation complete: ${cached} cached, ${failed} failed`,
        );
...
self.addEventListener("fetch", (event) => {
  ...
  const matchedPattern = CACHE_FIRST_PATTERNS.find((pattern) =>
  ...
  if (matchedPattern) {
    ...
    console.log(
      "[SW] 🎯 Cache-first pattern matched:",
      matchedPattern,
      "for",
      url.pathname,
    );
    ...
    event.respondWith(
      (async () => {
        ...
        const cacheMatchOptions = isDevHost
          ? undefined
          : {
              ignoreSearch: true,
              ignoreVary: true,
    ...
    (async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            "[SW] Network failed for navigation, checking for 404 fallback",
          );
          // Try to serve cached 404 page
          const notFoundCache =
            (await caches.match("/404/")) || (await caches.match("/404"));
          if (notFoundCache) {
            console.log(
              "[SW] Returning 404 page as offline fallback for:",
              url.pathname,
            );
          ...
          if (homepageCache) {
            console.log(
              "[SW] Returning homepage as final fallback for:",
              url.pathname,
            );
        ...
        console.error(
          "[SW] Failed to fetch and no cache available:",
          url.pathname,
        );
        throw err;
      }
