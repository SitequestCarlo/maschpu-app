...
*/
export type PipeMaterial = "acm" | "cin" | "cmc" | "con" | "cpr" | "stl" | "gir" | "hdpe" | "pvc" | "frp" | "rub" | "hdpe";
...
export function getHazenWilliamsCoefficient(material: PipeMaterial): number {
  switch (material) {
    case "acm": return 130; // Asbestzement
    case "cin": return 130; // Gusseisen
    case "cmc": return 140; // Zementmörtel-ausgekleidetes duktiles Gusseisenrohr
    case "con": return 140; // Beton
    case "cpr": return 135; // Kupfer
    case "stl": return 100; // Stahl
    case "gir": return 120; // Verzinktes Eisen
    case "hdpe": return 150; // Hochverdichtetes Polyethylen
    case "pvc": return 150; // Polyvinylchlorid
    case "frp": return 150; // Glasfaserverstärkter Kunststoff
    case "rub": return 150; // Gummi
    default: throw new Error("Unknown pipe material");
  }
}
...
export function getRequirerdPumpHead (
  ...
  const requiredPumpHead = nE / 1 + 0.002083 
  * Math.pow(100 / nC, 1.852) 
  * Math.pow(nQ, 1.852) / Math.pow(nD, 4.8704) * nL
...
export function getMaxPipeLength (
  ...
  const maxLength = (nH - (nE / 1)) / ((0.002083 * Math.pow(100 / nC, 1.852) * Math.pow(nQ, 1.852) / Math.pow(nD, 4.8704)))
  return maxLength
}
...
export function getMaxFlowRate (
  ...
  const maxFlowRate = Math.pow((nH - (nE / 1)) * Math.pow(nD, 4.8704) / (0.002083 * Math.pow(100 / nC, 1.852) * nL), 1 / 1.852)
  return maxFlowRate
}
...
export function getMinDiameter (
  ...
  const minDiameter = Math.pow((0.002083 * Math.pow(100 / nC, 1.852) * nL * Math.pow(nQ, 1.852)) / (nH - (nE / 1)), 0.2055)
  return minDiameter
}
