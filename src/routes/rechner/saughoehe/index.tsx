...
export const HeightCalc = component$(() => {
  ...
  const calcHeight = $((e: Event, form: HTMLFormElement) => {
    ...
    /** Durchflussmenge m³/S */
    const flow =
      parseFloat((form.querySelector("input#flow") as HTMLInputElement).value) /
      60000;
    /** Durchmesser Saugschläuche in m*/
    const hose_diam =
      parseInt(
        (form.querySelector("select#hose-diam") as HTMLSelectElement).value,
      ) / 1000;
    /** Anzahl Saugschläuche */
    const hose_amt = parseInt(
      (form.querySelector("input#hose-amt") as HTMLInputElement).value,
    );
    /** Länge Saugschlauch in m */
    const hose_length = parseFloat(
      (form.querySelector("input#hose-length") as HTMLInputElement).value,
    );
    /** Luftdruck in Pa */
    const air_p =
      parseFloat(
        (form.querySelector("input#air-p") as HTMLInputElement).value,
      ) *
      100 *
      1000;
    /** Wassertemperatur in °C */
    const water_temp = parseFloat(
      (form.querySelector("input#water-temp") as HTMLInputElement).value,
    );

    const safety =
      parseFloat(
        (form.querySelector("input#safety") as HTMLInputElement).value,
      ) /
        100 +
      1;
    ...
    const verl_dynamisch =
      (((1 / 2) * rho * Math.pow(v, 2)) / (rho * g)) * safety;
    ...
    const verl_reibung =
      ((lambda * (hose_length / hose_diam) * (rho / 2) * Math.pow(v, 2)) /
        (rho * g)) *
      safety;
    verlReib.value = verl_reibung;

    manometr.value =
      max_geodaet - verl_dampfdruck - verl_dynamisch - verl_reibung;
  });
  ...
  return (
    <div class={styles.tool}>
      ...
      <p>
        Wie hoch eine Pumpe ansaugen kann, hängt von vielen Faktoren ab. Mit
        diesem Rechner kann man sich eine grobe Einschätzung abholen.
      </p>
      ...
      <div class={styles.result}>
        <div>
          ...
          <span>
            <b>{Math.max(geodaet.value, 0).toFixed(2)}</b> m
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(manometr.value, 0).toFixed(2)}</b> m
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(verlDampf.value, 0).toFixed(2)}</b> m
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(verlReib.value + verlDyn.value, 0).toFixed(2)}</b> m
          </span>
        </div>
      </div>
    </div>
