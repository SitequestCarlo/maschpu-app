...
export function validateRequiredPumpPressure(input: HTMLInputElement, min: number, max: number, label: string): boolean {
...
export function showRequiredPumpPressureResult(form: HTMLFormElement): void {
    ...
    if (!validateRequiredPumpPressure(form.nFlow, 0, 9999999, "Capacity") ||
    ...
    const num = nE / 1 + 0.002083 * Math.pow(100 / nHazen, 1.85) *
        Math.pow(nQ, 1.85) / Math.pow(nD, 4.8655) *
        nL;
    const result = num.toFixed(2);
    if (!isNaN(Number(result)))
        form.nOut.value = result;
    else
        form.nOut.value = (window as any).$("#NoValueElement").val();
}

// Second section

export function validateMaximumPipeLength(input: HTMLInputElement, min: number, max: number, label: string): boolean {
    const message = `${label} has an invalid value: ${input.value}`;
    const str = input.value;
    for (let i = 0; i < str.length; i++) {
        const ch = str.substring(i, i + 1);
...
export function showMaximumPipeLengthResult(form: HTMLFormElement): void {
    if ((form.nFlowMaxPipe.value == null || form.nFlowMaxPipe.value.length === 0) ||
        (form.nDiamMaxPipe.value == null || form.nDiamMaxPipe.value.length === 0) ||
        (form.nOutMaxPipe.value == null || form.nOutMaxPipe.value.length === 0) ||
        (form.nElvMaxPipe.value == null || form.nElvMaxPipe.value.length === 0)) {
        return;
    }
    if (!validateMaximumPipeLength(form.nFlowMaxPipe, 0, 9999999, "Capacity") ||
        !validateMaximumPipeLength(form.nDiamMaxPipe, 0, 9999, "Diameter") ||
        !validateMaximumPipeLength(form.nOutMaxPipe, 0, 999999, "Pressure Difference") ||
    ...
    const num = (nO - (nE / 1)) / ((0.002083 * Math.pow(100 / nHazen, 1.85) * Math.pow(nQ, 1.85)) / (Math.pow(nD, 4.8655)));
    const result = num.toFixed(2);
    if (!isNaN(Number(result)))
        form.nLenMaxPipe.value = result;
    else
        form.nLenMaxPipe.value = (window as any).$("#NoValueElement").val();
}

// Third section

export function validateMaximumCapacity(input: HTMLInputElement, min: number, max: number, label: string): boolean {
    const message = `${label} has an invalid value: ${input.value}`;
    const str = input.value;
    for (let i = 0; i < str.length; i++) {
        const ch = str.substring(i, i + 1);
...
export function showMaximumCapacityResult(form: HTMLFormElement): void {
    if ((form.nOutMaxFlow.value == null || form.nOutMaxFlow.value.length === 0) ||
        (form.nDiamMaxFlow.value == null || form.nDiamMaxFlow.value.length === 0) ||
        (form.nLenMaxFlow.value == null || form.nLenMaxFlow.value.length === 0) ||
        (form.nElvMaxFlow.value == null || form.nElvMaxFlow.value.length === 0)) {
        return;
    }
    if (!validateMaximumCapacity(form.nOutMaxFlow, 0, 9999999, "Pressure Difference") ||
    ...
    const num = (Math.pow((((form.nOutMaxFlow.value - (nE / 1)) / (0.002083 * Math.pow(100 / nHazen, 1.85))) / nL) * Math.pow(nD, 4.8655), 0.54054)) / 4.403333;
    const result = num.toFixed(2);
    if (!isNaN(Number(result)))
        form.nFlowMaxFlow.value = result;
    else
        form.nFlowMaxFlow.value = (window as any).$("#NoValueElement").val();
}

// Fourth section

export function validateRequiredPipeDiameter(input: HTMLInputElement, min: number, max: number, label: string): boolean {
    const message = `${label} has an invalid value: ${input.value}`;
    const str = input.value;
    for (let i = 0; i < str.length; i++) {
        const ch = str.substring(i, i + 1);
...
export function showRequiredPipeDiameterResult(form: HTMLFormElement): void {
    if ((form.nFlowDiameter.value == null || form.nFlowDiameter.value.length === 0) ||
        (form.nLenDiameter.value == null || form.nLenDiameter.value.length === 0) ||
        (form.nOutDiameter.value == null || form.nOutDiameter.value.length === 0) ||
        (form.nElvDiameter.value == null || form.nElvDiameter.value.length === 0)) {
        return;
    }
    if (!validateRequiredPipeDiameter(form.nFlowDiameter, 0, 9999999, "Capacity") ||
        !validateRequiredPipeDiameter(form.nLenDiameter, 0, 9999, "Length") ||
        !validateRequiredPipeDiameter(form.nOutDiameter, 0, 999999, "Pressure Difference") ||
    ...
    const num = Math.pow((0.002083 * Math.pow(100 / nHazen, 1.85) * Math.pow(nQ, 1.85)) / ((nO - (nE / 1)) / nL), 0.2055) * 25.4;
    const result = num.toFixed(2);
    if (!isNaN(Number(result)))
        form.nDiamDiameter.value = result;
    else
        form.nDiamDiameter.value = (window as any).$("#NoValueElement").val();
}
