...
export const FlowCalc = component$(() => {
  ...
  const calcFlow = $((e: Event, form: HTMLFormElement) => {
    ...
    /** Durchflusseinheit */
    const unit = parseInt(
      (form.querySelector("select#unit") as HTMLSelectElement).value,
    ) as FlowUnit;
    ...
    <div class={styles.tool}>
          ...
          <div class={styles.selectWrapper}>
            <select required id="unit">
              <optgroup label="Liter">
                <option value={FlowUnit.LITERS_SECOND}>
                  l/s (Liter/Sekunde)
                </option>
                <option value={FlowUnit.LITERS_MINUTE} selected>
                  l/min (Liter/Minute)
                </option>
              ...
              <optgroup label="Kubikmeter">
                <option value={FlowUnit.CUBIC_SECOND}>
                  m³/s (Kubikmeter/Sekunde)
                </option>
                <option value={FlowUnit.CUBIC_MINUTE}>
                  m³/min (Kubikmeter/Minute)
                </option>
                <option value={FlowUnit.CUBIC_HOUR}>
                  m³/h (Kubikmeter/Stunde)
                </option>
              </optgroup>
            </select>
          </div>
      ...
      <div class={styles.result}>
        <div>
          ...
          <span>
            <b>{Math.max((m3h.value * 1000) / 60 / 60, 0).toFixed(2)}</b> l/s
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max((m3h.value * 1000) / 60, 0).toFixed(2)}</b> l/min
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(m3h.value * 1000, 0).toFixed(2)}</b> l/h
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(m3h.value / 60 / 60, 0).toFixed(2)}</b> m³/s
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(m3h.value / 60, 0).toFixed(2)}</b> m³/min
          </span>
        </div>
        ...
        <div>
          ...
          <span>
            <b>{Math.max(m3h.value, 0).toFixed(2)}</b> m³/h
          </span>
        </div>
      </div>
    </div>
