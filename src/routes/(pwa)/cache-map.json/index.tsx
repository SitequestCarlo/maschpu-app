...
export const onGet: RequestHandler = async (ev) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[cache-map] Found ${allFiles.length} files in dist directory`,
      );
      console.log(
        `[cache-map] Sample files:`,
        allFiles.filter((f) => f.includes("/build/")).slice(0, 10),
      );
      paths.push(...allFiles);
    } catch (err) {
    ...
    console.log(
      `[cache-map] Build files:`,
      uniquePaths.filter((p) => p.includes("/build/")).length,
    );
